...
typedef struct {
    int numeroJoueur;  // Numéro du joueur robot
    // Autres attributs spécifiques au joueur robot
} JoueurRobot;
...
// Fonction pour initialiser un joueur robot
JoueurRobot initJoueurRobot(int numero) {
    JoueurRobot joueur;
...
int gameActive = 1;
 ...
 printf("Carte %d: Numéro = %d, Têtes de bœufs = %d\n", k + 1, rangCartesSurTable[i].cartes[k].nombre, rangCartesSurTable[i].cartes[k].teteBoeuf);
    ...
    JoueurRobot robot = initJoueurRobot(1); // Initialisation du joueur robot avec le numéro 1



 }

 // Choix de carte par le joueur robot et envoi au serveur
    int choixCarte;
    choisirCarte(&robot, &choixCarte, sockfd);
    return 0;
}